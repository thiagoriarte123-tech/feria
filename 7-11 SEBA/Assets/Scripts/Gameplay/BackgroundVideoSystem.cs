...
public class BackgroundVideoSystem : MonoBehaviour
{
    ...
    void SetupVideoMaterial()
    {
        ...
        if (videoShader == null)
        {
            // Fallback a shader que siempre está disponible
        ...
        if (videoShader == null)
        {
            Debug.LogError("🎬 ❌ No se encontró ningún shader disponible para el video!");
            return;
        }
    ...
    IEnumerator LoadVideoAsync(string songFolderPath)
    {
        ...
        Debug.Log($"🎬 BackgroundVideoSystem: Iniciando carga de video para: {songFolderPath}");
        Debug.Log($"🎬 Application.isEditor: {Application.isEditor}");
        Debug.Log($"🎬 Application.streamingAssetsPath: {Application.streamingAssetsPath}");

        // Validar que el material esté creado
        if (videoMaterial == null || videoPlayer == null)
        {
            Debug.LogError("🎬 ❌ Sistema de video no inicializado correctamente (material o player null)");
        ...
        buildFix.LoadVideo(songFolderPath, (success) => {
            ...
            loadSuccess = success;
            videoLoaded = success;
            Debug.Log($"🎬 BuildFix callback recibido - Success: {success}");
        });
        ...
        {
            Debug.LogError($"❌ Error o timeout cargando video (timer: {timer}s, loadCompleted: {loadCompleted}, loadSuccess: {loadSuccess})");
        }
